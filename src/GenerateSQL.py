    ...
    for string in string_list:
        ...
        if count % limit == 0:
            ...
            with open(file_name, "w") as file:
                ...
                file.write(f"\n")
                file.write(f"UPDATE t_pa_pl_policy SET policy_no=CONCAT('{prefix}', policy_no), `PROPOSAL_STATUS` ='-1', `POLICY_STATUS` ='-1' WHERE `POLICY_NO` in ({temp});\n\n")
                file.write(f"UPDATE t_pa_pl_policy_l SET policy_no=CONCAT('{prefix}', policy_no),`PROPOSAL_STATUS` ='-1',`POLICY_STATUS` ='-1' WHERE `POLICY_NO` in  ({temp});\n\n")
                file.write(f"UPDATE t_pa_endo_endorsement SET endo_no=CONCAT('{prefix}', endo_no), policy_no=CONCAT('{prefix}', policy_no),`ENDO_STATUS` ='-1' WHERE `POLICY_NO` in  ({temp});\n\n")
                file.write(f"\n")
                file.write(f"use hdibrazil_tenant;\n")
                file.write(f"\n")
                file.write(f"UPDATE t_pa_settle_settlement  SET POLICY_NO=CONCAT('{prefix}', policy_no),ENDO_NO=CONCAT('{prefix}', ENDO_NO) WHERE policy_no in ({temp});\n\n")
                file.write(f"UPDATE t_pa_settle_settlement_item SET certificate_policy_no = CONCAT('{prefix}', certificate_policy_no) WHERE certificate_policy_no in  ({temp});\n\n")
                file.write(f"UPDATE t_pa_settle_settlement_item SET certificate_endo_no = REPLACE(certificate_endo_no,'{prefix}{prefix}','{prefix}') WHERE certificate_policy_no LIKE '%{prefix}%';\n\n")
                file.write(f"UPDATE t_pa_settle_settlement_item SET certificate_endo_no = CONCAT('{prefix}', certificate_endo_no) WHERE certificate_policy_no LIKE '{prefix}%' and certificate_endo_no IS NOT NULL AND ITEM_ID NOT IN ({not_in});\n\n")
                file.write(f"\n")
                file.write(f"DELETE FROM t_pa_settle_settlement_item it WHERE it.CERTIFICATE_ENDO_NO LIKE '-%' OR it.CERTIFICATE_POLICY_NO LIKE '-%';\n")
                file.write(f"\n")
...
def start():
    logger()
    content = read_json_file('data/invalidate2.json')
